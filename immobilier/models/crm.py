...
class crm(models.Model):
    ...
    #Champ pour filtrer les differents different type de contact dans le crm
    
    is_client = fields.Boolean(related='partner_id.client', store=True)
    
    is_prospect = fields.Boolean(related='partner_id.prospect', store=True)
    
    is_apporteur = fields.Boolean(related='partner_id.apporteur_affaires', store= True)
   ...
   #partner_id = fields.Many2one('res.partner', string='Customer', tracking=10, index=True,
    ...
    date_livraison = fields.Date('Date de livraison', required=True) 
    ...
    contrat_id = fields.Many2one('lb.contrat', string="Type de contrat", required=True)
    #contrat_type = fields.Many2many(related='nom_projet_ids.contrat_ids', string="type contrat projet")
